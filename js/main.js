;(function () {
	...
	$(function(){
		...
		/*function myFunction() {
			...
			var randd = Math.floor(Math.random() * (maxx - minn + 1) + minn); 
			
			for(var i = 0; i<(document.getElementsByClassName("popupp").length);i++){
				if(document.getElementsByClassName("popupp")[i].style.display = "block"){
					active = true
				}
			}
			console.log(rand, randd, active)
			if(active === false) {show(`popup${randd}`)}
